...
Release notes: [docs/releases/1.0.2.md](docs/releases/1.0.2.md)

### Fixed

- Prevented Windows console flashes from watcher child `node.exe`, the desktop Codex app-server wrapper, and proxy-detection `reg.exe` calls by using `CREATE_NO_WINDOW`.

## 1.0.1

Release notes: [docs/releases/1.0.1.md](docs/releases/1.0.1.md)

### Fixed

- Built the Windows watcher service host as a no-console process and stopped falling back to a visible console worker when it is not launched by the service manager.
- Hid Windows child processes used by self-update, dependency install, build, and repair commands.
- Avoided Windows `EBUSY` self-update failures by running watcher and manual update helpers outside the source tree before replacement.
- Treated missing GitHub Releases as "no published release found" instead of a hot-update failure.
- Ignored stale Release 404 watcher log blocks in the service health panel after the no-release state is recorded.
