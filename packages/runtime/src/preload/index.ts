...
let featureRuntimeStarted = false;
...
async function isPluginEnabled(): Promise<boolean> {
  try {
    const config = await ipcRenderer.invoke("codexpp:get-config") as { enabled?: boolean };
...
async function startFeatureRuntime(): Promise<void> {
...
function stopFeatureRuntime(): void {
  teardownTweakHost();
  featureRuntimeStarted = false;
  fileLog("feature runtime stopped");
}

function subscribePluginEnabledChanges(): void {
  ipcRenderer.on("codexpp:plugin-enabled-changed", (_event, payload) => {
    const enabled = payload && typeof payload === "object" && (payload as { enabled?: unknown }).enabled !== false;
    if (enabled) {
      void startFeatureRuntime().catch((e) => {
        fileLog("feature runtime restart FAILED", String((e as Error)?.stack ?? e));
      });
      return;
    }
    stopFeatureRuntime();
