import { app, BrowserView, BrowserWindow, MessageChannelMain, ipcMain, nativeTheme } from "electron";
...
interface CodexWindowServices {
  ...
  ) => { registerWindow?: (windowLike: CodexWindowLike) => void } | null;
  windowManager?: {
    registerWindow?: (
...
let activeHost: BrowserUiHost | null = null;
let activeOptions: BrowserUiServerOptions | null = null;
const bridgeRequests = new Map<string, BridgePendingRequest>();
const controlClients = new Set<WebSocketConnection>();

export function maybeStartBrowserUiServer(
  opts: Pick<BrowserUiServerOptions, "getWindowServices" | "log">,
): void {
...
export function startBrowserUiServer(opts: BrowserUiServerOptions): void {
  ...
  server.on("upgrade", (req, socket, head) => {
    handleUpgrade(req, socket as Socket, head).catch((error) => {
...
function installBrowserUiIpcHandlers(log: LogFn): void {
  ipcMain.removeAllListeners(BRIDGE_RESPONSE_CHANNEL);
  ipcMain.removeAllListeners(MESSAGE_FOR_VIEW_CHANNEL);
  ipcMain.removeAllListeners(WORKER_MESSAGE_CHANNEL);
  ipcMain.removeAllListeners(SYSTEM_THEME_CHANNEL);

  ipcMain.on(BRIDGE_RESPONSE_CHANNEL, (event, payload) => {
  ...
  ipcMain.on(MESSAGE_FOR_VIEW_CHANNEL, (event, message) => {
    if (!isBrowserUiHostSender(event.sender)) return;
    broadcastControl({ type: "message-for-view", message });
  });

  ipcMain.on(WORKER_MESSAGE_CHANNEL, (event, workerId, message) => {
    if (!isBrowserUiHostSender(event.sender)) return;
    if (typeof workerId !== "string") return;
    broadcastControl({ type: "worker-message", workerId, message });
  });

  ipcMain.on(SYSTEM_THEME_CHANNEL, (event, value) => {
    if (!isBrowserUiHostSender(event.sender)) return;
  ...
  process.once("exit", () => {
    for (const pending of bridgeRequests.values()) {
      clearTimeout(pending.timer);
      pending.reject(new Error("codex汉化增强plus版 browser UI server stopped"));
    }
...
async function browserIndexHtml(options: BrowserUiServerOptions): Promise<string> {
  const indexPath = join(webviewRoot(), "index.html");
  let html = relaxBrowserUiCsp(readFileSync(indexPath, "utf8"));
...
function relaxBrowserUiCsp(html: string): string {
  return html.replace(
...
function parseCspDirectives(content: string): Map<string, string> {
...
function formatCspDirectives(directives: Map<string, string>): string {
...
async function collectInitialState(options: BrowserUiServerOptions): Promise<InitialState> {
...
async function ensureBrowserUiHost(): Promise<BrowserUiHost> {
...
async function waitForWindowServices(options: BrowserUiServerOptions): Promise<CodexWindowServices> {
  const started = Date.now();
  while (Date.now() - started < 30_000) {
    const services = options.getWindowServices();
    if (
      services?.windowManager?.registerWindow &&
...
function callHiddenBridge(method: string, args: unknown[]): Promise<unknown> {
...
function bridgeMessagePortToWebSocket(port: Electron.MessagePortMain, ws: WebSocketConnection): void {
  let closed = false;
  const close = () => {
    if (closed) return;
    closed = true;
...
function broadcastControl(payload: unknown): void {
...
function browserBridgeScript(state: InitialState): string {
  return `
(() => {
  ...
  async function bridge(method, args = []) {
    ...
    if (!body.ok) throw new Error(body.error || "codex汉化增强plus版 browser bridge failed");
    return body.value;
  }
...
function hideVisibleCodexWindows(): void {
...
function acceptWebSocket(req: IncomingMessage, socket: Socket, head: Buffer): WebSocketConnection {
...
class WebSocketConnection {
  ...
  private textHandlers = new Set<(text: string) => void>();
...
function webviewRoot(): string {
  return join(process.resourcesPath, "app.asar", "webview");
}

function webviewFile(pathname: string): string | null {
  const cleanPath = decodeURIComponent(pathname).replace(/^\/+/, "");
  if (!cleanPath || cleanPath.includes("\0")) return null;
  const root = webviewRoot();
  const file = normalize(join(root, cleanPath));
...
function requireOptions(): BrowserUiServerOptions {
  if (!activeOptions) throw new Error("codex汉化增强plus版 browser UI server is not configured");
  return activeOptions;
}

function isBrowserUiHostSender(sender: Electron.WebContents): boolean {
  return !!activeHost && !activeHost.webContents.isDestroyed() && sender.id === activeHost.webContents.id;
}

function assertBridgeMethod(method: string): void {
...
function currentSystemThemeVariant(): string {
