...
export const MCP_MANAGED_START = "# BEGIN codex汉化增强plus版 MANAGED MCP SERVERS";
export const MCP_MANAGED_END = "# END codex汉化增强plus版 MANAGED MCP SERVERS";
const LEGACY_MCP_BRAND = ["CODEX", "++"].join("");
const LEGACY_MCP_MANAGED_START = `# BEGIN ${LEGACY_MCP_BRAND} MANAGED MCP SERVERS`;
const LEGACY_MCP_MANAGED_END = `# END ${LEGACY_MCP_BRAND} MANAGED MCP SERVERS`;
const MCP_MANAGED_MARKERS = [
  [MCP_MANAGED_START, MCP_MANAGED_END],
...
export function mergeManagedMcpBlock(currentToml: string, managedBlock: string): string {
  if (!managedBlock && !hasManagedMcpBlock(currentToml)) return currentToml;
...
export function stripManagedMcpBlock(toml: string): string {
  let next = toml;
  for (const [start, end] of MCP_MANAGED_MARKERS) {
...
function hasManagedMcpBlock(toml: string): boolean {
  return MCP_MANAGED_MARKERS.some(([start]) => toml.includes(start));
}
