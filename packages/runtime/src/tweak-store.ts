...
export function normalizeStoreEntry(input: unknown): TweakStoreEntry {
  ...
  return {
    ...
    archiveUrl: optionalGithubDownloadUrl((entry as { archiveUrl?: unknown }).archiveUrl),
...
export function storeArchiveUrl(entry: TweakStoreEntry): string {
  if (entry.archiveUrl) return entry.archiveUrl;
...
function optionalGithubDownloadUrl(value: unknown): string | undefined {
  if (typeof value !== "string" || !value.trim()) return undefined;
  const url = new URL(value);
