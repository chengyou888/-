...
test("buildManagedMcpBlock creates TOML entries and resolves local server scripts", () => {
  withTempDir((root) => {
    ...
    const serverScriptArg = JSON.stringify(join(tweakDir, "mcp-server.js"));
    assert.match(built.block, new RegExp(`args = \\[${escapeRegExp(serverScriptArg)}\\]`));
...
test("syncManagedMcpServers updates only the managed config block", () => {
  withTempDir((root) => {
    ...
    assert.match(afterFirst, /# BEGIN codex汉化增强plus版 MANAGED MCP SERVERS/);
    ...
    assert.doesNotMatch(afterSecond, /codex汉化增强plus版 MANAGED/);
    assert.doesNotMatch(afterSecond, /CODEX\+\+ MANAGED/);
  });
});

test("syncManagedMcpServers removes legacy managed config blocks", () => {
  withTempDir((root) => {
    const configPath = join(root, ".codex", "config.toml");
    mkdirSync(join(root, ".codex"), { recursive: true });
    const legacyBrand = ["CODEX", "++"].join("");
    writeFileSync(
      configPath,
      [
        `[mcp_servers.project-home]`,
        `command = "node"`,
        ``,
        `# BEGIN ${legacyBrand} MANAGED MCP SERVERS`,
        `[mcp_servers.native-widgets]`,
        `command = "node"`,
        `# END ${legacyBrand} MANAGED MCP SERVERS`,
        ``,
      ].join("\n"),
    );
    ...
    assert.match(after, /\[mcp_servers\.project-home\]/);
    assert.doesNotMatch(after, /native-widgets/);
    assert.doesNotMatch(after, /CODEX\+\+ MANAGED/);
  });
});
