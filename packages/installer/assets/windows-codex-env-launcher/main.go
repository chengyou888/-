...
func main() {
	...
	cmd := exec.Command(originalPath, os.Args[1:]...)
	cmd.Stdin = os.Stdin
...
func windowsSystemProxy() string {
	...
	enabledOutput, err := exec.Command("reg.exe", "query", key, "/v", "ProxyEnable").Output()
	if err != nil || !regexp.MustCompile(`(?i)\b0x1\b`).Match(enabledOutput) {
		return ""
	}
	serverOutput, err := exec.Command("reg.exe", "query", key, "/v", "ProxyServer").Output()
	if err != nil {
		return ""
	}
