...
function manifestTweakText(manifest) {
    if (STORE_TWEAK_ZH[manifest.id])
        return STORE_TWEAK_ZH[manifest.id];
    return null;
}
...
function localizeInstallationSource(source) {
    const label = source.kind === "github-source" ? "GitHub 源码安装" :
...
function localizeReleaseNotes(markdown) {
    ...
    if (!text)
        return "暂无发布说明。";
    if (!text.includes(["Codex", "++ 1.0.0 is the first stable release"].join("")) &&
...
function refreshComposerModelSourceLabel() {
    if (state.modelSourceLoading)
        return;
    state.modelSourceLoading = true;
    void electron_1.ipcRenderer.invoke("codexpp:get-active-agent-provider")
        .then(async (active) => {
        ...
        const config = await electron_1.ipcRenderer.invoke("codexpp:get-agent-provider-config", provider);
...
function syncComposerModelSourceLabel() {
    ...
    if (!label || !title)
        return;
    const button = document.querySelector('button[data-codex-intelligence-trigger="true"]');
    if (!button)
        return;
...
function findComposerModelNameSpan(button) {
    const leafSpans = Array.from(button.querySelectorAll("span"))
        .filter((span) => span.children.length === 0 && !!span.textContent?.trim());
    return (leafSpans.find((span) => span.className.includes("text-token-foreground")) ??
...
function onDocumentClick(e) {
    ...
    if (!(control instanceof HTMLElement))
        return;
    if (!localizeBackToAppLabel(control.textContent || ""))
        return;
    setTimeout(() => {
...
function renderConfigPage(sectionsWrap, subtitle) {
    ...
    void electron_1.ipcRenderer
        ...
        .catch((e) => {
        if (subtitle)
            subtitle.textContent = "无法加载已安装的 codex汉化增强plus版 版本。";
        card.textContent = "";
...
function renderAgentProvidersPage(sectionsWrap, subtitle) {
    ...
    pickerCard.appendChild(agentControlRow("当前模型来源", "默认沿用 Codex 自带模型；选择第三方后在下方配置 API Key 和模型。", picker));
    ...
    const renderSelected = (selection, options = {}) => {
        ...
        if (options.syncSelection) {
            void electron_1.ipcRenderer.invoke("codexpp:set-active-agent-provider", selection)
                .then(() => refreshComposerModelSourceLabel())
                .catch(() => undefined);
        }
        if (selection === "codex-native") {
            if (subtitle)
                subtitle.textContent = "使用 Codex 当前的 OpenAI 原生模型配置，不需要额外填写服务商信息。";
...
function renderCodexNativeProvider(sectionsWrap) {
    ...
    const card = roundedCard();
    card.appendChild(rowSimple("使用 Codex / OpenAI 原生模型", "当前会继续使用 Codex 自身的模型选择和鉴权配置；第三方接入不会覆盖它。"));
    section.appendChild(card);
    sectionsWrap.appendChild(section);
}
function readAgentProviderSelection() {
...
function renderAgentProviderPage(sectionsWrap, providerId, subtitle, options = {}) {
    ...
    void electron_1.ipcRenderer
        ...
        .catch((e) => {
        if (subtitle)
            subtitle.textContent = `无法加载 ${meta.label} 配置。`;
        settingsCard.textContent = "";
...
function renderAgentProviderConfig(settingsCard, providerId, config) {
    ...
    const refreshModels = async (options = {}) => {
        ...
        try {
            const result = await electron_1.ipcRenderer.invoke("codexpp:list-agent-provider-models", providerId, collect());
            ...
            if (options.autoTest) {
                if (result.models.length > 0)
                    ...
                    showAgentProviderTestDialog("测试失败", "服务商没有返回可选模型，暂时无法发送测试请求。请确认 API Key 权限、账号地域和 Base URL 是否匹配。", "error");
            }
        }
        catch (e) {
            setAgentModelOptions(modelSelect, [], "");
    ...
    const saveConfig = async (options = {}) => {
        ...
        const saved = await electron_1.ipcRenderer.invoke("codexpp:set-agent-provider-config", providerId, collect());
        if (savedStatus && !options.quiet) {
    ...
    const maybeAutoTest = async () => {
        ...
        if (!isAgentProviderReadyForAutoTest(providerId, current))
            return;
        ...
        if (fingerprint === lastAutoTestFingerprint)
            return;
        ...
        try {
            ...
            const result = await electron_1.ipcRenderer.invoke("codexpp:test-agent-provider", providerId, { prompt: DEFAULT_AGENT_TEST_PROMPT, config: current });
            if (result.sessionId)
            ...
            const activation = shouldAutoActivate
                ? await electron_1.ipcRenderer.invoke("codexpp:activate-agent-provider", providerId, current)
                : null;
            if (activation)
                refreshComposerModelSourceLabel();
            showAgentProviderTestDialog("测试成功", activation
                ? `${formatAgentProviderTestResult(result)}\n\n主界面接管：${activation.message}\n桥接地址：${activation.bridgeUrl ?? "Codex 原生"}\n配置文件：${activation.configPath}`
                : `${formatAgentProviderTestResult(result)}\n\n主界面接管：未自动切换。该测试不是本次手动输入触发，已避免旧页面状态覆盖当前 Codex 原生模型。`, "success");
        }
        catch (e) {
        ...
        saveTimer = setTimeout(() => {
            ...
            void saveConfig()
                ...
                if (options.refreshModels)
                    return refreshModels({ autoTest: options.autoTest });
                if (options.autoTest)
                    return maybeAutoTest();
                return undefined;
            })
                .catch((e) => {
                if (!savedStatus)
    ...
    settingsCard.appendChild(agentControlRow("启用入口", "关闭后保留配置，但测试请求会被阻止。", switchControl(enabled, async (next) => {
        enabled = next;
        await saveConfig({ quiet: true });
    })));
    settingsCard.appendChild(agentControlRow("认证模式", "桥接模式保留 Codex 官方登录态；纯 API 模式把 API Key 写入桌面 Codex 隔离 auth.json。", accessModeSelect));
    ...
    const modelControl = agentStackControl(modelSelect, agentInlineActions([refreshModelsButton, modelStatus]));
    if (providerId === "qwen") {
    ...
    advanced.body.appendChild(agentControlRow("采样与长度", "temperature 控制发散程度；最大输出限制本次回答长度。", agentInlineControls([
    ...
    docsActions?.appendChild(compactButton("打开文档", () => {
        void electron_1.ipcRenderer.invoke("codexpp:open-external", meta.docsUrl);
    }));
    if (meta.keyUrl) {
        docsActions?.appendChild(compactButton("申请 API Key", () => {
            void electron_1.ipcRenderer.invoke("codexpp:open-external", meta.keyUrl);
        }));
    }
...
function formatAgentProviderTestResult(result) {
    ...
    if (result.sessionId)
        meta.push(`session_id: ${result.sessionId}`);
    if (result.usage)
        meta.push(`usage: ${JSON.stringify(result.usage)}`);
    if (meta.length > 0)
...
function isAgentProviderReadyForAutoTest(providerId, config) {
    if (!config.enabled)
        return false;
    if (!isCompleteAgentApiKey(config.apiKey ?? ""))
        return false;
    if (!config.baseUrl?.trim())
        return false;
    if (providerId === "qwen" && config.mode === "app")
...
function maybePromptXiaobaiRegistration(providerId, config) {
    if (!shouldPromptXiaobaiRegistration(providerId, config))
        return;
    const label = agentProviderMeta(providerId).label;
    const accepted = window.confirm(`${label} 尚未填写 API Key。\n\n是否启用小白AI辅助自动注册 API？`);
    if (!accepted)
        return;
...
async function openXiaobaiAiToolboxForApiRegistration(providerId) {
    try {
        await electron_1.ipcRenderer.invoke("codexpp:open-xiaobai-toolbox", {
            provider: providerId,
            purpose: "api-registration",
        });
    }
    catch (e) {
        showAgentProviderTestDialog("无法打开小白AI工具箱", formatAgentProviderCaughtError(e), "error");
    }
}
...
function agentBaseUrlDescription(providerId) {
    if (providerId === "deepseek")
        return "默认追加 /chat/completions。";
    if (providerId === "zhipu")
        return "默认追加 /chat/completions，使用智谱 OpenAI 兼容接口。";
...
function pluginEnabledRow(config) {
    ...
    left.appendChild(desc);
    row.appendChild(left);
    row.appendChild(switchControl(config.enabled, async (next) => {
        await electron_1.ipcRenderer.invoke("codexpp:set-plugin-enabled", next);
...
function updateChannelRow(config) {
    ...
    if (config.updateChannel === "custom") {
        action?.appendChild(compactButton("编辑", () => {
...
function checkForUpdatesRow(config) {
    ...
    actions.appendChild(compactButton("立即检查", () => {
    ...
    actions.appendChild(compactButton("下载更新", () => {
        row.style.opacity = "0.65";
...
function watcherSummaryRow(health) {
    ...
    action.appendChild(compactButton("立即检查", () => {
...
function selfUpdateSummary(state) {
    if (!state)
        return "尚未运行过自动 codex汉化增强plus版 更新。";
    ...
    if (state.status === "failed")
        return `失败于 ${checked}。${target} ${state.error ?? "未知错误"}`;
    if (state.status === "updated")
        return `已更新于 ${checked}。${target} 来源：${source}。`;
    if (state.status === "up-to-date")
        return `已是最新 ${checked}。${target} 来源：${source}。`;
    if (state.status === "disabled")
        return `已跳过 ${checked}；自动刷新已关闭。`;
...
function uninstallRow() {
    const row = actionRow("卸载 codex汉化增强plus版", "复制卸载命令。完全退出 Codex 后，在终端中运行它。");
    const action = row.querySelector("[data-codexpp-row-actions]");
    action?.appendChild(compactButton("复制命令", () => {
        void electron_1.ipcRenderer
...
function reportBugRow() {
    const row = actionRow("Ai Open Tool", "打开 Ai Open Tool 获取反馈、支持和工具箱相关信息。");
    const action = row.querySelector("[data-codexpp-row-actions]");
    action?.appendChild(compactButton("打开 Ai Open Tool", () => {
        void electron_1.ipcRenderer.invoke("codexpp:open-external", AI_OPEN_TOOL_URL);
    }));
    return row;
}
...
function tweakStoreCard(entry) {
    ...
    if (entry.releaseUrl) {
        actions.appendChild(compactButton("版本", () => {
    ...
    if (entry.installed && !hasUpdate) {
        ...
        if (hasUpdate)
            actions.appendChild(storeStatusPill("有可用更新", "info"));
...
function renderTweaksPage(sectionsWrap) {
    ...
    if (state.listedTweaks.length === 0) {
        ...
        const card = roundedCard();
        card.appendChild(rowSimple("尚未安装插件", `把插件文件夹放入 ${tweaksPath()}，然后重载。`));
        section.appendChild(card);
        sectionsWrap.appendChild(section);
        return;
    }
    ...
    for (const p of state.pages.values()) {
        if (!pagesByTweak.has(p.tweakId))
            pagesByTweak.set(p.tweakId, []);
    ...
    for (const t of state.listedTweaks) {
        card.appendChild(tweakRow(t, sectionsByTweak.get(t.manifest.id) ?? [], pagesByTweak.get(t.manifest.id) ?? []));
    }
...
function tweakRow(t, sections, pages) {
    ...
    if (t.update?.updateAvailable && t.update.releaseUrl) {
        right.appendChild(compactButton("查看版本", () => {
...
function findContentArea() {
    ...
    if (reusablePanel?.parentElement)
        return reusablePanel.parentElement;
    const candidates = [];
    ...
    while (parent && depth < 8) {
        for (const child of Array.from(parent.children)) {
            ...
            if (!rightOfSidebar && !meaningfullyWiderThanSidebar)
                continue;
