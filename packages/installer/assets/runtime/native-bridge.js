...
class NativeBridge {
    ...
    loadModule(ctx, options) {
        ...
        if (kind !== "node-addon") {
            throw new Error(`${kind} native modules must be loaded through a .node Objective-C++ shim in codex汉化增强plus版 1.0.0`);
        }
    ...
    launchHelper(ctx, options) {
        ...
        if ((options.transport ?? "stdio") !== "stdio") {
            throw new Error("native helpers support only stdio transport in codex汉化增强plus版 1.0.0");
        }
        if ((options.restart ?? "never") !== "never") {
            throw new Error("native helper restart policies are not available in codex汉化增强plus版 1.0.0");
        }
    ...
    async createNativeInstance(ctx, kind, moduleId, factory, options) {
        ...
        if (typeof fn !== "function") {
            const label = moduleId ? `native module ${ctx.id}:${moduleId}` : "codex汉化增强plus版 native host";
    ...
    loadNativeHost(required) {
        ...
        if (!nativeHostPath || !(0, node_fs_1.existsSync)(nativeHostPath)) {
            const error = new Error("codex汉化增强plus版 native host is not installed");
        ...
        try {
            ...
            this.log("info", "loaded codex汉化增强plus版 native host", { path: nativeHostPath });
            return this.nativeHostExports;
        }
        catch (error) {
            this.nativeHostLoadError = error instanceof Error ? error : new Error(String(error));
            this.log("error", "failed to load codex汉化增强plus版 native host", this.nativeHostLoadError);
            if (required)
    ...
    readNativeHostCapabilities(host) {
        ...
        try {
            ...
            this.log("warn", "codex汉化增强plus版 native host capability probe failed", error);
            return {};
        }
    }
