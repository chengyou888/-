...
const WINDOWS_CODEX_ENV_LAUNCHER_ORIGINAL = "codexpp-codex-original.exe";
const WINDOWS_CODEX_ENV_LAUNCHER_MARKER = "codexpp-codex-env-launcher.json";
...
export async function install(opts: Opts = {}): Promise<void> {
  ...
  step("Runtime staged");
  const envLauncher = installWindowsCodexEnvLauncher(codex, paths.root);
  if (envLauncher) {
    step(
      envLauncher.changed
        ? `Desktop app-server env launcher installed: ${kleur.cyan(envLauncher.cliPath)}`
        : "Desktop app-server env launcher already installed",
    );
  }
...
interface WindowsCodexEnvLauncherResult {
  changed: boolean;
  cliPath: string;
  originalPath: string;
}

export function installWindowsCodexEnvLauncher(
  codex: CodexInstall,
  userRoot: string,
): WindowsCodexEnvLauncherResult | null {
  if (codex.platform !== "win32") return null;

  const cliPath = join(codex.resourcesDir, "codex.exe");
  if (!existsSync(cliPath)) return null;

  const originalPath = join(codex.resourcesDir, WINDOWS_CODEX_ENV_LAUNCHER_ORIGINAL);
  const markerPath = join(codex.resourcesDir, WINDOWS_CODEX_ENV_LAUNCHER_MARKER);
  const launcherPath = buildWindowsCodexEnvLauncher(userRoot);
  const launcherSha256 = sha256File(launcherPath);
  const currentSha256 = sha256File(cliPath);
  const marker = readWindowsCodexEnvLauncherMarker(markerPath);

  if (currentSha256 === launcherSha256 && existsSync(originalPath)) {
    return { changed: false, cliPath, originalPath };
  }

  if (
    !existsSync(originalPath) ||
    !marker ||
    currentSha256 !== marker.launcherSha256
  ) {
    copyFileSync(cliPath, originalPath);
  }

  copyFileSync(launcherPath, cliPath);
  writeFileSync(
    markerPath,
    JSON.stringify(
      {
        installedAt: new Date().toISOString(),
        launcherSha256,
        originalSha256: sha256File(originalPath),
        originalName: WINDOWS_CODEX_ENV_LAUNCHER_ORIGINAL,
      },
      null,
      2,
    ),
  ...
  return { changed: true, cliPath, originalPath };
}

export function restoreWindowsCodexEnvLauncher(codex: CodexInstall): boolean {
  if (codex.platform !== "win32") return false;
  const cliPath = join(codex.resourcesDir, "codex.exe");
  const originalPath = join(codex.resourcesDir, WINDOWS_CODEX_ENV_LAUNCHER_ORIGINAL);
  const markerPath = join(codex.resourcesDir, WINDOWS_CODEX_ENV_LAUNCHER_MARKER);
  if (!existsSync(originalPath)) return false;
  copyFileSync(originalPath, cliPath);
  rmSync(originalPath, { force: true });
  rmSync(markerPath, { force: true });
  return true;
}

function buildWindowsCodexEnvLauncher(userRoot: string): string {
  const launcherSrc = join(assetsDir, "windows-codex-env-launcher");
  const outDir = join(userRoot, "service-host");
  const outExe = join(outDir, "codexpp-codex-env-launcher.exe");
...
function readWindowsCodexEnvLauncherMarker(path: string): { launcherSha256?: string } | null {
  try {
    return JSON.parse(readFileSync(path, "utf8")) as { launcherSha256?: string };
  } catch {
    return null;
  }
}
