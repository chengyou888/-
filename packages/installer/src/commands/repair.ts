...
import { install, installWindowsCodexEnvLauncher, readCodexVersion, stageAssets } from "./install.js";
...
export async function repair(opts: Opts = {}): Promise<void> {
  ...
  if (isWatcherRepair(opts) && !isPluginEnabled(paths.configFile)) {
    if (!opts.quiet) console.log(kleur.yellow("codex-plusplus plugin switch is off; watcher repair skipped."));
    return;
  }
  ...
  if (state && !opts.force) {
    ...
    if (headerHash === state.patchedAsarHash) {
      const watcher = refreshWatcher(state.watcher, codex.appRoot, opts.quiet);
      refreshWindowsCodexEnvLauncher(codex, paths.root, opts.quiet);
...
function isPluginEnabled(configFile: string): boolean {
  if (!existsSync(configFile)) return true;
  try {
    const config = JSON.parse(readFileSync(configFile, "utf8")) as {
      codexPlusPlus?: { enabled?: boolean };
    };
    return config.codexPlusPlus?.enabled !== false;
  } catch {
    return true;
  }
}
...
function refreshWatcher(
  ...
  if (previous === "windows-service" && process.env.CODEX_PLUSPLUS_WATCHER === "1") {
    return previous;
  }
  try {
...
function refreshWindowsCodexEnvLauncher(
  codex: ReturnType<typeof locateCodex>,
  userRoot: string,
  quiet?: boolean,
): void {
  try {
    const result = installWindowsCodexEnvLauncher(codex, userRoot);
    if (result?.changed && !quiet) {
      console.log(kleur.green("Desktop app-server env launcher repaired."));
    }
  } catch (e) {
    if (!quiet) console.warn(kleur.yellow(`Desktop app-server env launcher repair failed: ${(e as Error).message}`));
  }
}
