...
export async function selfUpdate(opts: Opts = {}): Promise<void> {
  ...
  const repo = opts.repo ?? process.env.CODEX_PLUSPLUS_REPO ?? config.updateRepo ?? "chengyou888/-";
  const sourceRoot = findSourceRoot(here);
  const parent = dirname(sourceRoot);
  leaveSourceRootCwd(sourceRoot, parent);
  ...
  try {
    try {
      if (opts.watcher && config.enabled === false) {
        ...
        log(opts, "codex-plusplus plugin switch is off; watcher update skipped.");
        return;
      }
      ...
      if (!target) {
        ...
        log(opts, `No published codex-plusplus release was found for ${repo}; running repair only.`);
        runRepairIfRequested(opts, sourceRoot, parent);
        return;
      }
...
async function fetchLatestAnyRelease(repo: string): Promise<GitHubRelease | null> {
...
function leaveSourceRootCwd(sourceRoot: string, parent: string): void {
  if (process.platform !== "win32") return;
  try {
    const rel = relative(resolve(sourceRoot), resolve(process.cwd()));
    if (rel === "" || (!rel.startsWith("..") && !isAbsolute(rel))) {
      process.chdir(parent);
    }
  } catch {
    /* Best effort only; a locked source directory will still report a normal filesystem error. */
  }
}
