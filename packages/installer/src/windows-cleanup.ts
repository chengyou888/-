...
export const WINDOWS_WATCHER_SERVICE_NAME = "codex-plusplus-watcher";
...
export function buildWindowsManagedCleanupScript(input: {
  ...
  const cleanupPaths = [
    ...
    input.appData ? join(input.appData, "codex-plusplus", "service-host") : null,
  ...
  return [
    ...
    `$watcherService = '${WINDOWS_WATCHER_SERVICE_NAME}'`,
    "try { sc.exe stop $watcherService | Out-Null } catch {}",
    "$serviceQuery = sc.exe queryex $watcherService 2>$null",
    "$servicePid = ($serviceQuery | Select-String -Pattern 'PID\\s*:\\s*(\\d+)' | ForEach-Object { $_.Matches[0].Groups[1].Value } | Select-Object -First 1)",
    "if ($servicePid -and $servicePid -ne '0') { try { taskkill.exe /PID $servicePid /T /F | Out-Null } catch {} }",
    "try { sc.exe delete $watcherService | Out-Null } catch {}",
    "$watcherTasks = @(",
