...
export function showPatchFailedAlert(errorMessage: string): void {
  ...
  const button = showAlert({
    ...
    message:
      ...
      `${errorMessage}\n\n` +
      "Run codexplusplus repair from Terminal after Codex finishes updating, or open Ai Open Tool for support.",
    buttons: ["Dismiss", "Ai Open Tool"],
    defaultButton: "Dismiss",
    critical: true,
  });

  if (button === "Ai Open Tool") {
...
export function buildPatchFailureIssueUrl(errorMessage: string): string {
  ...
  return `${AI_OPEN_TOOL_URL}?${params.toString()}`;
}
